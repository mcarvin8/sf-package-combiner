...
_See code: [src/commands/sfpc/combine.ts](https://github.com/mcarvin8/sf-package-combiner/blob/v4.1.1/src/commands/sfpc/combine.ts)_
<!-- commandsstop -->
...
If every input is invalid or empty, the output will have no `<types>`. Use `--fail-on-empty` to guard against deploying an empty package:
